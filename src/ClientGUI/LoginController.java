...
public class LoginController {
    ...
    private void LoginToServer() {
		try{
			...
			if(result.next()){
				...
				ScreenController.removeScreen("Login");
		    } else {
    ...
    private void UpdateLastLoginTime(String usernameToUpdate) {
		try{
			Class.forName("com.mysql.jdbc.Driver");
			Connection con=DriverManager.getConnection("jdbc:mysql://"+prefs.get("server_address", "")+":3306/JavaChat?autoReconnect=true&useSSL=false","chat","7xsVuPeF1rCQOeo2");
			
			PreparedStatement updateEXP = (PreparedStatement) con.prepareStatement("update users set last_login='"+new Timestamp(System.currentTimeMillis())+"', last_server='"+prefs.get("server_address", "")+":"+prefs.get("server_port", "")+"' where user='"+usernameToUpdate+"'");
